...
export default function App() {
  return <MDC value={content} components={components} />
}
```
...
export default function ArticleCard() {
  ...
  return <MDC value={content} excerpt />
}
```
...
| `components` | `Record<string, ComponentType>` | Custom component mappings |
| `componentsManifest` | `(name: string) => Promise<ComponentModule>` | Dynamic component loader |
...
export default function Content({ content }: Props) {
  return <MDC value={content} className="prose" />
}
